...
Run these steps first for either local mode:
...
2. Optional: copy environment defaults if you want to set API or database values:

```bash
cp .env.example .env.local
```

3. Optional: prepare a live local PostgreSQL database if you do not want demo-mode data:
...
When the database setup step is skipped, the app falls back to demo data.

## Run Local Development Server

Use this when actively building, debugging, or changing code.
...
- `pnpm dev` is the correct mode for day-to-day development.
- First visits to routes can feel slower because Next.js compiles pages on demand in development mode.
- This slower first-load behavior is expected and does not reflect production performance.

## Run Local Production Server

Use this when you want to check the real user experience without development-mode overhead.
...
- This is the correct mode for checking whether slowness is caused by the app itself or by development-mode compilation.
- If the app feels slow in `pnpm dev` but acceptable in `pnpm start`, the difference is usually development-mode overhead rather than an inherently slow stack.
...
- `pnpm dev`
  Starts the Next.js development server. Use this after setup when actively developing.
- `pnpm build`
  Builds the app for production. Run this before `pnpm start`.
- `pnpm start`
  Runs the production build generated by `pnpm build`. Use this to test the production user experience locally.
- `pnpm test`
  Runs the Vitest test suite.
- `pnpm db:generate`
  Generates the Prisma client. Optional unless you are using a local PostgreSQL database.
- `pnpm db:push`
  Pushes the Prisma schema to a configured database. Optional unless you are using a local PostgreSQL database.
- `pnpm db:migrate`
  Runs a Prisma development migration.
- `pnpm db:seed`
  Seeds the PostgreSQL database using the Prisma seed script. Optional unless you are using a local PostgreSQL database.
